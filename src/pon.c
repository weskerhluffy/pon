...
#if 1
COMUN_FUNC_STATICA entero_largo_sin_signo pon_mul_mod(entero_largo_sin_signo a,
		entero_largo_sin_signo b, entero_largo_sin_signo c) {
...
COMUN_FUNC_STATICA entero_largo_sin_signo pon_exp_mod(entero_largo_sin_signo a,
		entero_largo_sin_signo p, entero_largo_sin_signo m) {
	if (!p) {
		return 1;
	}
	if (p & 1) {
...
COMUN_FUNC_STATICA entero_largo_sin_signo pon_rand(entero_largo_sin_signo max) {
	...
	caca_log_debug("max %llu num bins %llu num_rand %llu", max, num_bins,
			num_rand);

	entero_largo_sin_signo x;
	do {
		x = random();
		caca_log_debug("num rand %llu - defec %llu = %llu y x %llu", num_rand,
...
COMUN_FUNC_STATICA entero_largo_sin_signo pon_rand_intervalo(
		entero_largo_sin_signo min, entero_largo_sin_signo max) {
	caca_log_debug("rand intervalo min %llu max %llu", min, max);
	return (entero_largo_sin_signo) min + pon_rand(max - min);
}

COMUN_FUNC_STATICA bool pon_prueba_miller_rabbit(entero_largo_sin_signo n) {
	entero_largo_sin_signo a = pon_rand_intervalo(2, n - 2);
	caca_log_debug("rand a %llu inter %llu-%llu", a, 2, n-2);
	entero_largo_sin_signo d = n - 1;
	while (!(d & 1)) {
		d >>= 1;
	}
	caca_log_debug("d %llu", d);
	entero_largo_sin_signo x = pon_exp_mod(a, d, n);
	caca_log_debug("x %llu", x);
...
COMUN_FUNC_STATICA bool pon_es_primo(entero_largo_sin_signo n, natural k) {
	if (n <= 1 || n == 4) {
		return falso;
	}
	if (n <= 3) {
...
COMUN_FUNC_STATICA void pon_main() {
	natural t = 0;
	scanf("%u\n", &t);
	while (t--) {
		entero_largo_sin_signo n = 0;
		scanf("%llu\n", &n);
		caca_log_debug("probando %llu", n);
